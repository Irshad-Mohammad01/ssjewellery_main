...
export const Home = () => {
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 min-h-screen pb-16">
      ...
      <div 
        className="relative h-[320px] sm:h-[480px] md:h-[540px] overflow-hidden bg-black mt-16 mb-8"
        style={{
          width: '100vw',
          position: 'relative',
          left: '50%',
      ...
      >
        ...
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center px-4">
          <motion.span
            ...
            className="text-[10px] sm:text-xs font-bold tracking-[0.25em] text-[#D4A75F] uppercase mb-4"
          >
            {language === 'hi' ? 'सच्ची विलासिता और शिल्प कौशल' : 'Pure Luxury & Craftsmanship'}
          </motion.span>
          ...
          <motion.h2
            ...
            className="font-serif font-black text-2xl sm:text-4xl md:text-5xl text-white tracking-tight leading-tight max-w-3xl mb-6"
            style={{ textShadow: '0 4px 18px rgba(0,0,0,0.6)' }}
          >
            {language === 'hi' 
              ? 'हर आभूषण में एक शाश्वत विरासत छिपी है' 
              : 'Timeless Legacies Handcrafted Into Every Single Masterpiece'
            }
          </motion.h2>
          ...
          <motion.p
            ...
            className="text-xs sm:text-sm text-slate-350 max-w-lg leading-relaxed mb-8"
          >
            {language === 'hi'
              ? 'हमारे हस्तनिर्मित उत्कृष्ट कृतियों के साथ सोने, हीरे और रत्नों की शुद्धता और सुंदरता का अनुभव करें।'
              : 'Experience the absolute purity and brilliance of ethically sourced gold, diamonds, and precious gemstones in designs built to last generations.'
            }
          </motion.p>
          ...
          >
            <Link
              ...
              onClick={(e) => {
                ...
                const allProductsHeading = document.getElementById('all-products-heading');
                if (allProductsHeading) {
                  allProductsHeading.scrollIntoView({ behavior: 'smooth' });
                }
              }}
              className="px-8 py-3.5 bg-[#D4A75F] hover:bg-[#BF934B] text-slate-950 font-bold text-xs uppercase tracking-widest rounded-full transition-transform hover:scale-105 shadow-xl flex items-center gap-2"
            >
              {language === 'hi' ? 'संग्रह का अन्वेषण करें' : 'Explore the Collection'}
